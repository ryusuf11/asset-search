  ...
  <div class="d-flex" style="gap: 40px;">
    ...
    <template v-else>
      <div class="d-flex flex-column" style="width: calc(100vw - 340px)">
        ...
        <AssetContainer assetType="lottie" :fetchItem="fetchItem" :jsonFile="true" />
      </div>
    </template>
  </div>
</template>
...
const { data: searchData, status } = useAsyncData(
  'search',
  async () => {
    const { data: keywords } = await getKeyword('lottie', {
...
const totalItems = computed(() => {
  return rawItem.value?.response?.aggregations?.assets?.lotties_count || 0;
})

const title = computed(() => {
  return `${totalItems.value} Lottie Animations`
})

const description = computed(() => {
  return `${totalItems.value} lottie animations exclusively selected by our designer community.`
})
