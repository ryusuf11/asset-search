<template>
  ...
  <div class="d-flex" style="gap: 40px;">
    ...
    <template v-else>
      <AssetContainer :fetchItem="fetchItem" />
    </template>
  </div>
</template>
...
const { data: searchData, status } = useAsyncData(
  'search',
  async () => {
    const { data: keywords } = await getKeyword('illustration', {
...
const totalItems = computed(() => {
  return rawItem.value?.response?.aggregations?.assets?.illustrations_count || 0;
})

const title = computed(() => {
  return `${totalItems.value} Illustrations`
})

const description = computed(() => {
  return `${totalItems.value} illustrations exclusively selected by our designer community.`
})
